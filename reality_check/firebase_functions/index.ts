 ...
 */
export const evaluateStreaks = functions.pubsub.schedule("0 0 * * *").timeZone("America/New_York").onRun(async (context) => {
  const now = new Date();
  ...
  for (const groupDoc of groupsSnapshot.docs) {
    ...
    for (const uid of members) {
      ...
      const currentLimit = userData?.current_limit || 120;
      const nextDayLimit = userData?.next_day_limit || currentLimit;
      ...
      let minutesUsed = 0;
      if (logSnap.exists) {
        minutesUsed = logSnap.data()?.minutes_used || 0;
      }
    ...
    if (memberCounts >= 2 && dailySaintUid) {
      ...
      const currentSaintDays = saintSnap.data()?.days_at_num1 || 0;
    ...
    if (now.getDay() === 0) { // Sunday
      ...
      if (memberCounts >= 2 && weeklySaintUid) {
        ...
        const currentWeeks = wSaintSnap.data()?.weeks_at_num1 || 0;
